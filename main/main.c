...
static const uint8_t digit_font[11][8] = {
    // 0
    {0b00111100, 0b01100110, 0b01101110, 0b01110110, 
...
static void lcd_reset_and_backlight(void) {
    ...
    vTaskDelay(pdMS_TO_TICKS(120));  // Display requires min 120ms after reset
...
static void draw_vline(uint16_t x, uint16_t y, uint16_t h, Color color) {
    ...
    // Allocate buffer for the exact number of pixels (3 bytes per pixel)
    ...
    spi_transaction_t t = {
        .length    = height * 24,  // 8 bits * 3 colors * height
        .tx_buffer = buffer,
        .user      = (void*)1,
    };
    spi_device_polling_transmit(spi_dev, &t);
...
static void draw_line(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, Color color) {
    ...
    memset(buffer, 0, sizeof(buffer)); // black background

    // Bresenham line into buffer
    ...
    // Send the buffer as one windowed transfer
...
static void lcd_fill_color(Color color) {
    ...
    for (size_t i = 0; i < total_chunks; i++) {
        ...
        spi_transaction_t t = {
            .length = lines * display.width * 24,  // 24 bits per pixel
...
static void lcd_set_brightness(uint8_t percent) {
    ...
    // Simple PWM control of backlight (ESP32 doesn't have hardware PWM on all pins)
...
static void lcd_draw_char(uint16_t x, uint16_t y, char c, FontSize scale, Color fg, Color bg) {
    if (c < '0' || c > '9') c = '0' + 10; // Use empty character for non-digits
    
    uint8_t digit = c - '0';
    uint16_t width = 8 * scale;
    uint16_t height = 8 * scale;
    
    // Set window for the scaled digit
    lcd_set_window(x, y, x + width - 1, y + height - 1);
    
    // Calculate buffer size needed (width * height * 3 bytes per pixel)
...
static void lcd_draw_number(uint16_t x, uint16_t y, int number, FontSize scale, Color fg, Color bg) {
    ...
    lcd_draw_string(x, y, buffer, scale, fg, bg);
}
...
void app_main(void) {
    ...
    // 1) Reset & backlight with proper timing
    lcd_reset_and_backlight();

    // 2) Preallocate DMA buffer
